...
from configuration import LLM, client
...
paraphrase_prompt = "Here is a youtube transcript of a player playing geoguessr, in which he is trying to guess the location based on the current views. There are totally five rounds. The youtuber starts with prologue, 5 rounds of games, and epilogue. Please split the scripts of 5 rounds of games, find useful clues in each round, and return me only a python list of the paraphrased clues (like xxx brands of cars, court style buildings, etc. Instead of using the player's point of view, state the clues in the scene in a neutral tone without mentioning the player) in each round in the transcripts with punctuation, and exclude the other parts. The clues should not showing the correctness of the final results. Also, the player sometimes get the answer correct, or get it wrong. You should determine it based on his word. Your should only return a list of python dict without '\n', like [{'transcript': clues in the first round, 'is_correct': True}, {...}] (Use double quotation marks instead, and make sure the keys are correct. The value of 'is_correct' should be true or false). The transcript is as follows:\n"
